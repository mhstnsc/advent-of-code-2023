...
abstract class MainBase(day: Int) {
  ...
  def main(args: Array[String]): Unit = {
    Try(Source.fromResource(s"day-$day-test.txt").getLines().toList).toOption.orElse(
      Try(Source.fromResource(s"attic/day-$day-test.txt").getLines().toList).toOption
    ).foreach { smallInput =>
      println(s"test result = ${run(smallInput)}")
    }

    Try(Source.fromResource(s"day-$day-small.txt").getLines().toList).toOption.orElse(
    ...
    val inputFile = Try(Source.fromResource(s"day-$day.txt").getLines().toList).toOption.orElse(
